@extends('layouts.app')
@section('title', 'Регистрация партнера — BiletOnline')

@section('content')
<div class="max-w-md mx-auto my-6 bg-white border border-zinc-200 p-8">
    <h2 class="text-xl font-bold tracking-tight mb-6 uppercase">Регистрация: Партнер</h2>
    ...
    <form action="{{ route('register.partner') }}" method="POST" class="space-y-4">
        @csrf
        <div>
            <label class="block text-xs uppercase tracking-wider text-zinc-500 font-semibold mb-1">Название площадки</label>
        ...
        <div>
            <label class="block text-xs uppercase tracking-wider text-zinc-500 font-semibold mb-1">Адрес площадки</label>
            <input type="text" name="address" value="{{ old('address') }}" required class="w-full border border-zinc-300 p-2 text-sm focus:outline-none focus:border-zinc-900 bg-zinc-50">
        </div>
        <div>
            <label class="block text-xs uppercase tracking-wider text-zinc-500 font-semibold mb-1">Контактный номер</label>
